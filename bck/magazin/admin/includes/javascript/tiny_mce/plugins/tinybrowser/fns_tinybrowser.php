<?php
function os_sql_regcase($item,$encoding='auto')
{ 
...
if($level==0 && is_dir($root.$tree.$branch))
	...
	foreach($filetypes as $filetype)
	   {
   	$filenum = $filenum + count(glob($root.$tree.$branch.os_sql_regcase($filetype),GLOB_NOSORT));
   	}
...
while (($dirname = readdir($dh)) !== false)
	...
	if($dirname != '.' && $dirname != '..' && is_dir($root.$tree.$branch.$dirname) && $dirname != '_thumbs')
		...
		foreach($filetypes as $filetype)
		   {
			$filenum = $filenum + count(glob($root.$tree.$branch.$dirname.'/'.os_sql_regcase($filetype),GLOB_NOSORT));
			}
		$indent = '';
