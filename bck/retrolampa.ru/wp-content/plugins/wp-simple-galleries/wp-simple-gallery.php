...
class wpsimplegallery {
    ...
    private $thumbnail_size_w = 140;
    private $thumbnail_size_h = 90;
    ...
    private function __construct() {
        $this->thumbnail_size_w = 140; //wpsg_of_get_option('wpsimplegallery_thumb_width');
        $this->thumbnail_size_h = 90; //wpsg_of_get_option('wpsimplegallery_thumb_height');
        ...
        add_image_size('wpsimplegallery_slide', 635, 407, true);
    ...
    private function slide($id, $post_id) {
    	$info = get_posts(array('p' => $id, 'post_type' => 'attachment'));
    	$url = wp_get_attachment_url($id);
    	
    	$width = 635; // $this->thumbnail_size_w;
    	$height = 407; // $this->thumbnail_size_h;
    	$image = array(
    			WPSIMPLEGALLERY_URL . 'timthumb.php?src=' . $url . '&q=85&w=' . $width . '&h=' . $height,
    			$width,
    			$height
    ...
    private function gallery($post_id = false) {
        ...
        if (is_array($gallery) && count($gallery) > 0) {
        	$html = '<div id="wpsimplegallery_container"><div class="flexslider"><ul class="clearfix slides">';
        	
        	foreach ($gallery as $thumbid) {
        		$html .= $this->slide($thumbid, $post_id);
        	}
        	
        	$html .= '</ul></div><ul id="wpsimplegallery" class="clearfix">';
