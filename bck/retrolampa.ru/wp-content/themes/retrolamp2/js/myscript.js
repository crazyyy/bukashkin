$(function(){
   ...
   // $('#wpsimplegallery_container').prepend("<div class='flexslider'>"+$('#wpsimplegallery_container').html()+"</div><div class='clear'></div>");
   // $('.flexslider > #wpsimplegallery').removeAttr("id").attr("class", "slides");
   // $('.slides img').removeAttr("width").removAattr("height");
   // $('.fancybox').addClass("fancybox");
    
    $(".flexslider").flexslider({
    	manualControls: $("#wpsimplegallery li"),
    	directionNav: false
    });
    /*
    $('#wpsimplegallery_container').removeAttr("id").addClass("flexslider");
    $('#wpsimplegallery').removeAttr("id").attr("class", "slides");
    $(".flexslider").flexslider();
    */
    $(".slides a").fancybox();
    ...
    setTimeout(function() {
    	column_height( $(".description") );
    	column_height( $(".ui-tabs-panel") );
    }, 10);
    
    function column_height( el ) {
        var mainDivs = $(el); //Получаем все элементы с классом description
        var maxHeight = 0;
