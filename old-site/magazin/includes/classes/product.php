...
class product {
	...
	{
		$reviews_query = osDBquery("select count(*) as total from ".TABLE_REVIEWS." r, ".TABLE_REVIEWS_DESCRIPTION." rd where r.products_id = '".$this->pID."' and r.reviews_id = rd.reviews_id and rd.languages_id = '".$_SESSION['languages_id']."' and rd.reviews_text !=''");
	...
	function getReviews() {
		...
		if (os_db_num_rows($reviews_query, true)) {
			...
			while ($reviews = os_db_fetch_array($reviews_query, true)) {
				$row ++;
				$data_reviews[] = array ('AUTHOR' => $reviews['customers_name'], 'DATE' => os_date_short($reviews['date_added']), 'RATING' => os_image('themes/'.CURRENT_TEMPLATE.'/img/stars_'.$reviews['reviews_rating'].'.gif', sprintf(TEXT_OF_5_STARS, $reviews['reviews_rating'])), 'TEXT' => os_break_string(htmlspecialchars($reviews['reviews_text']), 60, '-<br />'));
	...
	function buildDataArray(&$array, $image='thumbnail') 
    {
		global $osPrice, $main;
    		...
    		$products_price = $osPrice->GetPrice($array['products_id'], $format = true, 1, $array['products_tax_class_id'], $array['products_price'], 1);
