...
function get_cache ($filename)
{
	...
	{	
		 ...
		 {
		    //если нет файла с кэшем - создаем его
			 switch($filename) 
			 {
                case 'category':
                   set_category_cache();
	...
	{
	 /*   if (is_file(_CACHE.'system/configuration.php') && filesize(_CACHE.'system/configuration.php') > 0)
	    {
	         include(_CACHE.'system/configuration.php');		 			 
	    }
		else
		{
		     set_configuration_cache('set_define'); //не только создаетс€ кэш, но и обь€вл€ютс€ константы configuration
		}*/
	}
}
...
{
   //get_cache('configuration'); //получение configuration констант из кэша
   get_cache('default');
...
function set_category_cache ()
{
      ...
      if (os_db_num_rows($categories_query,true)) 
	  {
           ...
           {
	             $foo[$categories['categories_id']] = $categories['parent_id'];	
           } 
      }

      save_cache('category', $foo);
...
function save_cache ($filename, $cache, $method = 'file')
{
	...
	if ($method == 'file')
	{
	      ...
	      $fp = @fopen(_CACHE.'system/'.$filename.'.php', "w"); //очищаем существующий файл кзша
          if ($fp) 
	      {
               @fwrite($fp, $_cache);
          }
...
function get_categories_info ($id)
{
 ...
 //возвращает массив обратный $category_cache
...
//переворачивает массив. почти array_flip()
